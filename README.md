...
- 클라이언트 컴포넌트만 번들링한 js를 돌려주고 , 다시 Hydration 과정을 진행
...
- 일반 컴포넌트 props 로 전달받는 방법 또는 Context 를 이용하는 방버븡로 활용.

### App Router 에서는

- 서버 컴포넌츠라면 마음대로 데이터를 패칭할 수 있도록 적용함.
...
export default async function Home() {
  ...
  const resRandomGoods: GoodDataType[] = await resRandom.json();
...
- /src/componets/random-goods.tsx 생성
...
- search 는 사용자가 무엇을 검색할지 모르기 때문에 데이터 패칭으로 부적합.
...
- `{cache : "no-store"}` : 데이터를 매번 새로 가져옴 : 보관하지 말아라
- `{cache : "force-cache"}` : 데이터를 캐시에 저장하고 매번 가져옴 : 보관해라
- `{next : {revalidate : 숫자}}` : 요청시 숫자초 동안만 유지하고 갱신
- `{next : {tag : ['num']}}` : 특정 태그로 강제 요청
...
- 이후로 요청시 사전 랜더링 없이 바로 Cache 된 내용을 리턴
